...
void MainWindow::cancelTimerSlot() {
    ...
    this->timerEdit->setEnabled(true);
    this->timerContentEdit->setReadOnly(false);
    this->timerContentEdit->setEnabled(true);
    this->countdownUnitComboBox->setEnabled(true);
...
void MainWindow::setTimerSlot() {
    ...
    if (str == "" && str != "0")
        ...
        if (ok == true && dec != 0) {
            int seconds = 0;
            if(this->countdownUnitComboBox->currentData().toString() == this->UNIT_MINUTE){
                seconds = dec*60;//min => sec
            }else if(this->countdownUnitComboBox->currentData().toString() == this->UNIT_SECOND){
                seconds = dec;
            }

            this->timerEdit->setReadOnly(true);
            this->timerEdit->setEnabled(false);
            this->timerContentEdit->setReadOnly(true);
            this->timerContentEdit->setEnabled(false);
            this->countdownUnitComboBox->setEnabled(false);
            ...
            this->timer->start(seconds * 1000);
            // int QTimer::start ( int msec, bool sshot = FALSE )
            //如果sshot为真，这个定时器将只会被激活一次，否则它将会持续到它被停止。
        } else {
            QMessageBox::about(this, tr("Countdown Reminder"), tr("Please enter a countdown time"));
        }
    }
}
...
void MainWindow::copyToClip() {
    ...
    for (int i = 0; i < this->ipaddr_num; i++) {
        if (send == p[i].button) {
            ...
            QMessageBox::information(this, tr("Success"), tr("Copy successfully") + "\nIP:" + str);
            break;
        }
    }
}
...
MainWindow::MainWindow(QApplication *app, QWidget *parent)
    ...
    labelss = new QLabel(tr("Countdown time"));
    ...
    QVariant min;
    min.setValue(this->UNIT_MINUTE);
    QVariant sec;
    sec.setValue(this->UNIT_SECOND);
    this->countdownUnitComboBox = new QComboBox;
    this->countdownUnitComboBox->addItem("min(s)", min);
    this->countdownUnitComboBox->addItem("second(s)", sec);
    tabLayout2->addWidget(this->countdownUnitComboBox, 2, 2, 1, 1);
